...
export default async function ProfileUpdateHandler(req, res) {
    try {
        ...
        const response = await fetch(backendApiUrl('/users/profile_interaction/update_my_profile/'), {
            method: "PATCH",
            headers: {
        ...
        if (role) {
            const roleResponse = await fetch(backendApiUrl('/users/profile_interaction/my-role'), {
                method: "PATCH",
                headers: {
                    "Content-Type": "application/json",
